	...
	public void checkMove(int playerIdx, int[] cardIdx) {
		...
		//the move is valid
			...
			if (passFlag == 1) { //not pass
				...
				String newMsg = "{"+playerHand.getType()+"} ";
				playerHand.print();
				for (int g = 0; g < playerHand.size(); g++) {
					newMsg += "[" + playerHand.getCard(g) + "]";
				}
				ui.printMsg(newMsg);
				System.out.println();
			}
			...
			if (endOfGame()) {
				...
				for (int i = 0; i < numOfPlayers; i++) {
					...
					if (i != currentPlayerIdx) {
						...
						ui.printMsg("Player "+ i +" has "+ playerList.get(i).getCardsInHand().size() +" in hand.");
					}
					//the winner
					else {
						System.out.println("Player "+ currentPlayerIdx +" wins the game.");
						ui.printMsg("Player "+ currentPlayerIdx +" wins the game.");
					}
